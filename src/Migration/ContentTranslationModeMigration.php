...
/**
 * Initialises the content translation mode of existing language configurations.
 *
 * Every non-default language of an existing installation becomes "connected",
 * which is exactly how those installations behaved before free mode existed. A
 * value an editor already chose is preserved, invalid values are normalised, no
 * translation, source or free record is created, changed or deleted, and running
 * the migration again changes nothing.
 */
final class ContentTranslationModeMigration extends AbstractMigration
{
    private const TABLE = 'tl_inline_language';
    private const COLUMN = 'contentTranslationMode';
    ...
    {
        return $this->label('contaoMultilingualPagetreeContentModeMigration', self::class);
    }
    ...
    public function run(): MigrationResult
    {
        ...
        foreach ($this->recordsToNormalise() as $record) {
            $this->connection->update(
                self::TABLE,
                [self::COLUMN => ContentTranslationMode::Connected->value],
                ['id' => $record['id']],
            );
            ++$updated;
        }

        return $this->createResult(true, sprintf($this->label('contaoMultilingualPagetreeContentModeMigrated', '%d'), $updated));
    }
    ...
    private function recordsToNormalise(): array
    {
        ...
        foreach ($records as $record) {
            ...
            if (is_string($value) && null !== ContentTranslationMode::tryFrom($value)) {
                continue;
            }
